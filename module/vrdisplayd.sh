#!/system/bin/sh
# Следит за DisplayPort и включает внешний дисплей, как только очки воткнули.

MODDIR=${MODDIR:-${0%/*}}
case "$MODDIR" in ""|.|"$0") MODDIR=/data/adb/modules/vr_display_mode;; esac
. "$MODDIR/common.sh"

POLL_INTERVAL=2

# Логический дисплей появляется не мгновенно после того, как коннектор
# отрапортовал connected, поэтому пробуем несколько раз.
try_enable() {
    i=0
    while [ $i -lt 10 ]; do
        out=$(enable_external_display)
        case "$out" in
            *"enabled display"*)
                log "дисплей включён: $out"
                # Пульт может подняться позже самого дисплея, поэтому
                # привязываем указатель несколько раз с паузой.
                j=0
                while [ $j -lt 5 ]; do
                    pout=$(bind_pointers)
                    case "$pout" in
                        *"pointer bound"*)
                            log "указатель привязан: $pout"
                            break
                            ;;
                    esac
                    j=$((j + 1))
                    sleep 2
                done
                [ $j -eq 5 ] && log "указатель привязать не удалось: $pout"
                return 0
                ;;
        esac
        i=$((i + 1))
        sleep 1
    done
    return 1
}

# Отпечаток набора устройств ввода. По его изменению видно, что мышь или
# пульт воткнули (или переподключили) уже после того, как дисплей поднялся.
input_fingerprint() {
    grep -c . /proc/bus/input/devices 2>/dev/null
    grep "^N: Name=" /proc/bus/input/devices 2>/dev/null | sort | cksum
}

# Один демон на систему. pkill по имени здесь ненадёжен: он не всегда
# срабатывает из-за SELinux, а после ручных перезапусков экземпляры копились.
PIDFILE=/data/adb/vr_display_mode.pid
if [ -f "$PIDFILE" ]; then
    OLD=$(cat "$PIDFILE" 2>/dev/null)
    if [ -n "$OLD" ] && [ "$OLD" != "$$" ] && [ -d "/proc/$OLD" ]; then
        kill -9 "$OLD" 2>/dev/null && log "остановлен прежний демон ($OLD)"
    fi
fi
echo $$ > "$PIDFILE"

log "демон: старт"
prev=""
prev_inputs=""
enabled=no          # дисплей этой сессии уже включён
fail_logged=no      # о неудаче сказано, повторяем молча

while true; do
    if dp_connected; then
        cur=connected
    else
        cur=disconnected
    fi

    if [ "$cur" != "$prev" ]; then
        log "DisplayPort: $cur"
        prev_inputs=$(input_fingerprint)
        if [ "$cur" = disconnected ]; then
            enabled=no
            fail_logged=no
        fi
    fi

    if [ "$cur" = "connected" ] && [ "$enabled" = no ]; then
        # Включаем дисплей в обоих режимах. Это программный эквивалент
        # кнопки "каст" в системном диалоге, и без него очки не поднимают
        # аудиоусилитель: звука нет совсем. В режиме гарнитуры модуль
        # VR Headset Mode погасит картинку сразу после появления звука.
        #
        # Повтор на каждом цикле, пока не выйдет. Раньше попытка была
        # ровно одна, на смене состояния коннектора: если дисплей не
        # поднялся за отведённые попытки, демон больше не пробовал до
        # физического перетыка очков — а в режиме гарнитуры это означало
        # отсутствие звука совсем. После первого успеха не повторяем:
        # иначе включение спорило бы с гашением от соседнего модуля.
        if try_enable; then
            enabled=yes
            fail_logged=no
        elif [ "$fail_logged" = no ]; then
            log "включить дисплей не удалось: $out — повторяю, пока очки на шине"
            fail_logged=yes
        fi
    elif [ "$cur" = "connected" ]; then
        # Дисплей на месте: следим за появлением новых указывающих устройств,
        # чтобы мышь, воткнутая позже очков, тоже попала на внешний экран.
        inputs=$(input_fingerprint)
        if [ "$inputs" != "$prev_inputs" ]; then
            log "набор устройств ввода изменился — перепривязка указателя"
            pout=$(bind_pointers)
            case "$pout" in
                *"pointer bound"*) log "указатель привязан: $pout" ;;
                *)                 log "привязка не выполнена: $pout" ;;
            esac
            prev_inputs=$inputs
        fi
    fi

    prev=$cur
    sleep "$POLL_INTERVAL"
done
